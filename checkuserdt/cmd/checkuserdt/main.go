...
func (a *App) handleHeartbeat(w http.ResponseWriter, r *http.Request) {
	...
	fillIfEmpty(&s.VPNState, firstNonEmpty(q.Get("vpn_state"), q.Get("vpnState")))
	fillIfEmpty(&s.ConfigID, firstNonEmpty(q.Get("config_id"), q.Get("configId")))
	fillIfEmpty(&s.ConfigName, firstNonEmpty(q.Get("config_name"), q.Get("configName")))
	fillIfEmpty(&s.ConfigCategory, firstNonEmpty(q.Get("config_category"), q.Get("category")))
	fillIfEmpty(&s.LocalIP, firstNonEmpty(q.Get("local_ip"), q.Get("localIp")))
	fillIfEmpty(&s.NetworkName, firstNonEmpty(q.Get("network_name"), q.Get("networkName")))
	...
	// Sem isso, ele ficaria aparecendo por alguns minutos ate expirar o TTL.
	if isOfflineVPNState(s.VPNState) {
		...
		writeJSON(w, http.StatusOK, map[string]any{"status": "offline", "session": s})
		return
	}
...
func (a *App) handleDisconnect(w http.ResponseWriter, r *http.Request) {
	...
	if s.Username == "" && s.UUID == "" && s.DeviceID == "" {
		writeJSON(w, http.StatusBadRequest, map[string]any{"status": "error", "message": "informe username, uuid ou device_id"})
		return
	}
	if s.PublicIP == "" {
		s.PublicIP = clientIP(r)
